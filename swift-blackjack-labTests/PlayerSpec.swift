        ...
        describe("Player") {
            ...
            describe("description") {
                it("returns a readout of the Player instance") {
                    ...
                    expect(description.lowercased().contains("player"))
                    expect(description.lowercased().contains("cards"))
                    expect(description.lowercased().contains("handscore"))
                    expect(description.lowercased().contains("blackjack"))
                    expect(description.lowercased().contains("busted"))
                    expect(description.lowercased().contains("stayed"))
                    expect(description.lowercased().contains("tokens"))
                }
            }
            ...
            describe("canPlaceBet") {
                it("returns true if the tokens is equal to or greater than the bet") {
                    expect(player.canPlaceBet(amount: 10)).to(beTrue())
                    expect(player.canPlaceBet(amount: 100)).to(beTrue())
                }
                
                it("returns false if the tokens is less than the bet") {
                    expect(player.canPlaceBet(amount: 101)).to(beFalse())
                }
            }
